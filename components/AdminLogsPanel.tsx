...
const TYPE_COLOR: Record<string, string> = {
  ...
  member_joined: "text-muted",
  member_left: "text-muted",
...
export default function AdminLogsPanel({ token }: { token: string }) {
  ...
  return (
    <div className="bg-panel border border-border rounded-lg divide-y divide-border">
      {logs === undefined && <p className="text-sm text-muted px-4 py-3">Loading logs...</p>}
      {logs?.length === 0 && <p className="text-sm text-muted px-4 py-3">No activity yet.</p>}
      {logs?.map((log) => (
        <div key={log._id} className="px-4 py-2.5 flex items-start justify-between gap-4">
          <p className="text-sm text-text">
            <span className={`text-[10px] font-medium uppercase mr-2 ${TYPE_COLOR[log.type] ?? "text-muted"}`}>
