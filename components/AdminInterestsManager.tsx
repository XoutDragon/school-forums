...
export default function AdminInterestsManager({ token }: { token: string }) {
  ...
  return (
    <div className="space-y-6">
      <form onSubmit={handleAdd} className="bg-panel border border-border rounded-lg p-4 flex gap-2 items-end">
        ...
        <div>
          ...
          <select
            ...
            className="rounded-lg bg-panel2 border border-border px-3 py-2 text-sm text-text outline-none focus:border-accent"
          >
            {CATEGORIES.map((c) => (
        ...
        <button
          type="submit"
          disabled={submitting}
          className="rounded-lg bg-accent text-white text-sm font-medium px-4 py-2 hover:opacity-90 disabled:opacity-50"
        >
          Add
        </button>
      </form>

      {interests === undefined && <p className="text-sm text-muted">Loading...</p>}
          ...
          <div key={value}>
            ...
            <div className="flex flex-wrap gap-2">
              {items.map((item) => (
                <span
                  key={item._id}
                  className="flex items-center gap-2 bg-panel border border-border rounded-full pl-3 pr-2 py-1 text-sm text-text"
                >
                  {item.label}
                  <button
